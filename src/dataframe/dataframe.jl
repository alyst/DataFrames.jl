...
upgraded_scalar_length(df::DataFrame, row_inds::Colon) = (ncol(df) == 0) ? 1 : nrow(df)
upgraded_scalar_length{T<:Real}(df::DataFrame, row_inds::AbstractArray{T}) = length(row_inds)
upgraded_scalar_length(df::DataFrame, row_inds::AbstractArray{Bool}) = sum(row_inds)

function upgrade_scalar(df::DataFrame, v::Any, row_inds::Any)
    n = upgraded_scalar_length(df, row_inds)
